...
import type { OtherIncome, UserRole } from "@/types/database";
  ...
  if (rows.length === 0) {
    return (
      <div className="flex h-32 items-center justify-center rounded-[10px] border border-dashed border-gray-200 text-sm text-gray-400">
        Belum ada pemasukan lain-lain.
      </div>
    );
  }

  function canManage(row: OtherIncome) {
          ...
          {rows.map((row) => (
            <tr key={row.id}>
              <td className="px-4 py-3 whitespace-nowrap text-gray-500 font-mono">
                {formatDateShort(row.income_date)}
              </td>
              <td className="px-4 py-3 whitespace-nowrap">
                <span className="inline-flex rounded-full bg-good-soft px-2 py-0.5 text-xs font-semibold text-good">
                  {row.source}
                </span>
              </td>
